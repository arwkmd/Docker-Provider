...
private:
		...
		if (!response.empty() && response[0])
		{
			for (int i = 0; i < cJSON_GetArraySize(response[0]); i++)
			{
				...
				if (entry)
				{
					...
					{
						string mylog = "The container has no RepoTags :" + cJSON_GetObjectItem(entry, "Id")->valuestring;
						ofstream myfile;
						myfile.open("/var/opt/microsoft/omsagent/log/providerlogs.txt", std::ios_base::app);
						myfile << mylog.c_str() << endl;
						myfile.close();
	...
	static void ObtainContainerConfig(Container_ContainerInventory_Class& instance, cJSON* entry)
	{
		...
		{
			string mylog = "Attempt in ObtainContainerConfig to get container" + cJSON_GetObjectItem(entry, "Id")->valuestring + "config information returned null";
			ofstream myfile;
			myfile.open("/var/opt/microsoft/omsagent/log/providerlogs.txt", std::ios_base::app);
			myfile << mylog.c_str() << endl;
			myfile.close();
	...
	static void ObtainContainerState(Container_ContainerInventory_Class& instance, cJSON* entry)
	{
		...
		if (state)
		{
			...
			{
				exitCode = 128;
				string mylog = "Container " + cJSON_GetObjectItem(entry, "Id")->valuestring +" returned negative exit code";
				ofstream myfile;
				myfile.open("/var/opt/microsoft/omsagent/log/providerlogs.txt", std::ios_base::app);
				myfile << mylog.c_str() << endl;
				myfile.close();
		...
		{
			ofstream myfile;
			string mylog = "Attempt in ObtainContainerState to get container " + cJSON_GetObjectItem(entry, "Id")->valuestring +" state information returned null";
			myfile.open("/var/opt/microsoft/omsagent/log/providerlogs.txt", std::ios_base::app);
			myfile << mylog.c_str() << endl;
			myfile.close();
	...
	static void ObtainContainerHostConfig(Container_ContainerInventory_Class& instance, cJSON* entry)
	{
		...
		{
			string mylog = "Attempt in ObtainContainerHostConfig to get container " + cJSON_GetObjectItem(entry, "Id")->valuestring + " host config information returned null";
			ofstream myfile;
			myfile.open("/var/opt/microsoft/omsagent/log/providerlogs.txt", std::ios_base::app);
			myfile << mylog.c_str() << endl;
			myfile.close();
	...
	static Container_ContainerInventory_Class InspectContainer(string& id, map<string, vector<string> >& nameMap)
	{
		...
		if (!response.empty() && response[0])
		{
			...
			string mylog = "Getting namemap.count for container " + id.c_str();
			ofstream myfile;
			myfile.open("/var/opt/microsoft/omsagent/log/providerlogs.txt", std::ios_base::app);
			myfile << mylog.c_str() << endl;
			myfile.close();
		...
		{
			string mylog = "Attempt in InspectContainer to inspect " + id.c_str() + " returned null";
			ofstream myfile;
			myfile.open("/var/opt/microsoft/omsagent/log/providerlogs.txt", std::ios_base::app);
			myfile << mylog.c_str() << endl;
			myfile.close();
...
public:
	...
	static vector<Container_ContainerInventory_Class> QueryAll()
	{
		...
		for (set<string>::iterator i = containerIds.begin(); i != containerIds.end(); ++i)
		{
			// Set all data
			string id = string(*i);
			string mylog = "in QueryAll Creating containerinventory instance for containter: " + id.c_str();
			ofstream myfile;
			myfile.open("/var/opt/microsoft/omsagent/log/providerlogs.txt", std::ios_base::app);
			myfile << mylog.c_str() << endl;
			myfile.close();
...
void Container_ContainerInventory_Class_Provider::EnumerateInstances(Context& context, const String& nameSpace, const PropertySet& propertySet, bool keysOnly, const MI_Filter* filter)
{
	...
	catch (...)
	{
		...
		myuberfilenew.open("/var/opt/microsoft/omsagent/log/providerlogs.txt", std::ios_base::app);
